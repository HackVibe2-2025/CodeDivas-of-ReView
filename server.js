...
app.post('/api/entries', (req, res) => {
  try {
    ...
    // Get session userId if available
    ...
    const stmt = `INSERT INTO entries (user_id, apps, screen_time, reflection, tags, created_at) 
                  VALUES (?, ?, ?, ?, ?, ?)`;

    db.run(
      stmt,
      [finalUserId, JSON.stringify(apps), screenTime, reflection.trim(), JSON.stringify(tags), localTime],
      function (err) {
    if (err) {
        ...
        console.log(`📝 User ${finalUserId || 'Anonymous'} saved entry (ID: ${this.lastID})`);
        res.json({
          success: true,
          id: this.lastID,
...
app.get('/api/entries', (req, res) => {
  try {
    ...
    console.log('GET /api/entries - Session userId:', sessionUserId);
    console.log('GET /api/entries - Session:', req.session);
    
    let query, params;
    
    if (sessionUserId) {
      // If user is logged in, get only their entries
      query = 'SELECT * FROM entries WHERE user_id = ? ORDER BY created_at DESC';
      params = [sessionUserId];
    } else {
      // If no session, get all entries (for anonymous users)
    ...
    db.all(query, params, (err, rows) => {
      ...
      console.log(`👤 ${sessionUserId ? `User ${sessionUserId}` : 'Anonymous user'} requested entries - Found ${entries.length} entries`);
...
// Get entries by specific user ID (for testing)
app.get('/api/entries/:userId', (req, res) => {
  const userId = req.params.userId;
  db.all(`SELECT * FROM entries WHERE user_id = ? ORDER BY created_at DESC`, [userId], (err, rows) => {
    if (err) {
